...
?>
		...
		<div id="xhanch_my_twitter">
			...
			<?php if($scroll_mode){ ?><div style="max-height:<?php echo $scroll_h; ?>px;overflow:auto"><?php } ?>
			<ul>
			...
			<?php if($scroll_mode){ ?></div><?php } ?>
						
			<?php echo convert_smilies(html_entity_decode(get_option("my_twitter_text_footer"))); ?>

			<?php if (get_option("my_twitter_credit")){ ?>
				<div class="credit"><a href="http://xhanch.com/wp-plugin-my-twitter/" rel="section" title="My Twitter - A free WordPress plugin to display your latest tweets from Twitter" target="_blank">My Twitter</a>, by <a href="http://xhanch.com/" rel="section" title="Developed by Xhanch Studio" target="_blank">Xhanch</a></div>
			<? }?>
		</div>
<?php		
		echo $after_widget;
...
?>
		...
		<table>
			...
			<tr>
				<td width="150"><label for="my_twitter_scroll_area_height">Area Height</label></td>
