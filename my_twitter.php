...
?>
		...
		<div class="my_twitter">
			...
			<? foreach($res as $row){ ?>
				<div class="tweet">
					<a href="<?=$row['author_url']; ?>"><img class="avatar" src="<?=$row['author_img']; ?>" alt="Avatar"/></a> <?=$row['tweet']; ?> <?=$row['timestamp']; ?>
				</div>
				<div class="clear"></div>
			<? } ?>
		</div>
			...
			<tr>
				...
				<td>
					<select id="my_twitter_date_format" name="my_twitter_date_format" style="width:100%">
						<? foreach($arr_date_format as $fmt_val=>$fmt_ex){ ?>
							<option value="<?=$fmt_val; ?>" <?=($fmt_val==$date_format)?'selected="selected"':''; ?>><?=$fmt_ex; ?></option>
						<? } ?>
					</select>
				</td>
			</tr>
